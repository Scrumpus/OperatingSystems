...
public class CPU implements Runnable
{
	...
	public static final int CLOCK_FREQ = 5; // how many CPU cycles elapse between clock interrupts
	...
	/**
	 * This variable stores a reference to the CPU's interrupt controller
	 */
	...
	/**
	 * This variable counts how many CPU cycles have elapsed for the
	 * simulation
	 */
	 ...
	 * getTicks
	 * 
	 * @return current value of m_ticks
	 */
	public int getTicks()
	{
		return m_ticks;
	 ...
	 * addTicks
	 * 
	 * @param numTicks the number of ticks to add to m_ticks
	 */
	public void addTicks(int numTicks)
	{
		m_ticks = m_ticks +  numTicks;
	}
	 ...
	 * 
	 * @return -1 if no value to pop, or the value that has been popped
	 */
	public int pop() {
		if (getSP() <= getBASE() + getLIM())
		{
			int val = m_MMU.read(getSP());
			setSP(getSP()+1);
			return val;
		}
		return -1;
	...
	public void run()
	{
		...
		while(true){
			...
			int instruction[] = m_MMU.fetch(m_registers[PC]); 
			
			// Check to see if any devices have generated interrupts.
			// If so, we must handle them before executing any instructions.
			...
			case BNE:
				...
				//Goto an address given by a register, if the value of one register is less than another
			case BLT:
				//Check that PC address is in memory bounds
				if(!isMemAddressInRange(instruction[3]+getBASE()))
					m_TH.interruptIllegalMemoryAccess(instruction[1]+getBASE());
			...
			case PUSH:
				if (!push(m_registers[instruction[1]]))
				{
					m_TH.interruptIllegalInstruction(instruction);
				}
				break;
	...
	{
		m_TH = th;
		m_MMU.registerTrapHandler(m_TH);
	}
